...
export default function EditPost() {
	...
	const [values, setValues] = useState({ title: post.title, description: post.body });

	const user = useSelector((state: any) => state.user);

    const success = () => toast.success("Post editado correctamente", { autoClose: 1000 });
    const error = () => toast.error("Error al editar el post", { autoClose: 1000 });
	...
	const handlePost = (event: any) => {
        ...
        setLoading(true);
		editPost(user.token, values.title, values.description, user.id, post.id)
			.then(() => {
				setLoading(false);
                success();
	...
	return (
		<form onSubmit={handlePost}>
            ...
            <div className="heading text-center font-bold text-2xl m-5 text-gray-800">
				<h1 className="text-3xl">Editar Post {post.id}</h1>
			</div>
